    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p class="fw-bold">First Name:<span class="fw-normal"> {{ $user->firstname }}</span></p>
                <p class="fw-bold">Last Name:<span class="fw-normal"> {{ $user->lastname }}</span></p>
...
<div class="modal fade" id="edit{{ $user->id }}" tabindex="-1" aria-hidden="true" aria-labelledby="editModal">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ auth()->check() && auth()->user()->type === 'Manager' ? route('manager.manageusers.update', $user->id) : route('admin.manageusers.update', $user->id) }}" method="POST">
                    @method('PATCH')
                    @csrf
...
<div class="modal fade" id="delete{{ $user->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ auth()->check() && auth()->user()->type === 'Manager' ? route('manager.manageusers.delete', $user->id) : route('admin.manageusers.delete', $user->id)  }}" method="POST">
                    @method('DELETE')
                    @csrf
                    <h4 class="text-center">Are you sure you want to remove User?</h4>
                    <h5 class="text-center">Name: {{ $user->firstname }} {{ $user->lastname }}</h5>
                    <div class="modal-footer">
                        <button type="button" class="btn-danger" data-bs-dismiss="modal">Close</button>
